...
public class AlgoSPAM{
	...
	public void runAlgorithm(StringBuffer input, double minsupRel) throws IOException {
		// create an object to write the file
		//writer = new BufferedWriter(new FileWriter(outputFilePath)); 
		// initialize the number of patterns found
		patternCount =0; 
	 ...
	 * @throws IOException 
	 */
	private void spam(StringBuffer input, double minsupRel) throws IOException{
		...
		try {
			...
			StringBuffer reader = input;
			String thisLine;
			int bitIndex =0;
			
			Scanner scan = new Scanner(reader.toString()); // I have named your StringBuilder object sb
			/*while (scan.hasNextLine() ){
			 String oneLine = scan.nextLine();
			 System.out.println(oneLine.length());
			}*/
			// for each line (sequence) in the file until the end
			while (scan.hasNextLine() ){//(thisLine = reader.readLine()) != null)
		...
		System.out.println("size : "+sequencesSize.size());
		if(minsup ==0){
			minsup =1;
		}
		...
		try {
			...
			Scanner scan = new Scanner(reader.toString());
